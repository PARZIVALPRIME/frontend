...
// Cap mobile resolution to keep fill-rate low on dense phone displays.
const mobileMaxDpr =
  typeof window !== "undefined" ? Math.min(window.devicePixelRatio || 1, 1.5) : 1.5;

export const MODES: { id: SocMode; icon: string; label: string }[] = [
...
export function AppUI({ sceneComponent: SceneComp, quality }: UiProps) {
  ...
  // Desktop = full rendering pipeline. Mobile = lighter, mobile-friendly defaults.
  const dpr: [number, number] = quality === "desktop" ? [1, 2] : [1, mobileMaxDpr];
    ...
    <div className="relative h-screen w-screen overflow-hidden bg-[#08090e] font-sans text-white">
      ...
      <Canvas
        ...
        gl={{
          ...
          toneMappingExposure: quality === "desktop" ? 1.1 : 1.45,
          powerPreference: quality === "desktop" ? "high-performance" : "low-power",
        }}
        // Adaptive performance: drop resolution while interacting on mobile, restore when idle.
        performance={quality === "mobile" ? { min: 0.5, max: 1, debounce: 200 } : { min: 0.75 }}
        className="absolute inset-0"
      >
        <QualityContext.Provider value={quality}>
          <Suspense fallback={null}>
            <SceneEl t={t} showLabels={showLabels} selected={selected} setSelected={setSelected} mode={mode} />
        ...
        <div className="flex items-center gap-0.5 rounded-full border border-white/8 bg-black/40 p-0.5 backdrop-blur-xl">
          <button
            ...
            className={`rounded-full px-3 py-1 text-[10px] font-semibold transition ${
              t === 0 ? "bg-white/90 text-black shadow" : "text-white/45 hover:text-white/75"
            }`}
          >
            Assembled
          </button>
          <button
            onClick={() => setT(1)}
            className={`rounded-full px-3 py-1 text-[10px] font-semibold transition ${
              t === 1 ? "bg-white/90 text-black shadow" : "text-white/45 hover:text-white/75"
            }`}
          >
            Exploded
          </button>
      ...
      <div className="pointer-events-none absolute bottom-1.5 left-1/2 -translate-x-1/2 z-10">
        <span className="font-mono text-[8px] tracking-[0.15em] text-white/15">
          22×18u die · 3nm EUV FinFET · ~12B transistors · {quality === "desktop" ? "desktop render" : "mobile render"}
        </span>
      </div>
    </div>
  );
